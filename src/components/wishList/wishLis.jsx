...
function WishList({ id }) {
    ...
    function WishClick (id) {
        const wish = document.getElementById(`wishList-${id}`)
        
        if(localStorage.getItem(wishid) != null){
            localStorage.removeItem(wishid)
            wish.classList.remove('wish-listClicked')
        } else {
            wish.classList.add('wish-listClicked')
            localStorage.setItem(wishid, 'clicked')
        }
    ...
    return (
        <button className="wishlist" id={`wishList-${id}`} onClick={() =>WishClick(id)}>
